...
const DisplayReckoner = () => {
  ...
  const fetchSites = async () => {
    try {
      ...
      const res = await axios.get("http://3.211.76.159:5000/api/reckoner/sites");
      if (res.data.success) {
  ...
  const fetchSiteInfo = async (poNumber) => {
    try {
      ...
      const res = await axios.get(
        `http://3.211.76.159:5000/api/reckoner/sites/${poNumber}`
      );
      if (res.data.success) {
  ...
  const fetchReckonerData = async () => {
    try {
      setLoading(true);
      const res = await axios.get("http://3.211.76.159:5000/api/reckoner/reckoner/");
  ...
  const handleSubmit = async (rec_id) => {
    try {
      setSubmitting(true);
      await axios.patch(
        `http://3.211.76.159:5000/api/reckoner/completion_status/${rec_id}`,
        editingData
      );
