...
const MaterialDispatch = () => {
  ...
  const fetchAssignments = async (pd_id, site_id) => {
    try {
      ...
      const response = await axios.get(
        `http://3.211.76.159:5000/api/material/assignments?pd_id=${pd_id}&site_id=${site_id}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post("http://3.211.76.159:5000/api/material/dispatch-material", payload);
