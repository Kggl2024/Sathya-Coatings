...
const Dashboard = () => {
  ...
  const calculateProgressData = () => {
    if (!poTotals || !completionEntries || completionEntries.length === 0)
      return { percentage: 0, totalCompletedArea: 0, totalPoQuantity: 0 };
    const totalCompletedArea = completionEntries
      .filter((cat) => cat.category_id === selectedCategory?.value)
      .flatMap((cat) => cat.subcategories)
      .flatMap((subcat) => subcat.entries_by_date)
      .flatMap((dateEntry) => dateEntry.entries)
      .filter((entry) => !selectedDescription || entry.desc_id == selectedDescription.value)
      .reduce((sum, entry) => sum + (entry.area_added || 0), 0);
    const totalPoQuantity = poTotals.subcategory_totals
      .find((cat) => cat.category_id === selectedCategory?.value)
      ?.descriptions
      .filter((desc) => !selectedDescription || desc.desc_id == selectedDescription.value)
      .reduce((sum, desc) => sum + desc.subcategories.reduce((s, sc) => s + (sc.po_quantity || 0), 0), 0) || 0;
    const percentage =
      totalPoQuantity === 0
        ? 0
        : (totalCompletedArea / totalPoQuantity) * 100;
  ...
  const prepareDailyExpenseTrendChartData = () => {
    ...
    const sortedDates = expenseDetails.expenses_by_date.sort((a, b) => a.expense_date.localeCompare(b.expense_date));
    const allDates = sortedDates.map(e => e.expense_date);
    const datasets = [];
    ...
    if (expenseDataType.value === "category") {
      ...
      categories.forEach((category, index) => {
        const data = allDates.map(date => {
          // Approximate daily data by distributing category total (backend limitation)
          const categoryTotal = expenseDetails.expenses_by_category.find(c => c.expense_category_name === category)?.total_expense || 0;
          const dateEntry = expenseDetails.expenses_by_date.find(e => e.expense_date === date);
          return dateEntry ? categoryTotal / expenseDetails.expenses_by_date.length : 0;
        });
        datasets.push({
          label: category,
          data,
      ...
      descriptions.forEach((desc, index) => {
        const data = allDates.map(date => {
          // Approximate daily data by distributing description total (backend limitation)
          const descTotal = expenseDetails.expenses_by_work_description.find(d => d.desc_name === desc)?.total_expense || 0;
          const dateEntry = expenseDetails.expenses_by_date.find(e => e.expense_date === date);
          return dateEntry ? descTotal / expenseDetails.expenses_by_date.length : 0;
        });
        datasets.push({
          label: desc,
          data,
